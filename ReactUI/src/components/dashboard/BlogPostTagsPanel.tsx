...
import { BlogPostTagsIcon } from '@/components/dashboard/DashboardIcons'
import {
  AdminField,
  adminInputClass,
...
export function BlogPostTagsPanel() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!window.confirm(t('dashboard.blogPostTags.deleteConfirm'))) return
        ...
        <div className="space-y-3">
          ...
          <ul className="divide-y divide-border rounded-sm border border-border">
            {items.map((item) => (
              ...
              >
                <p className="text-sm font-semibold text-text">{item.tagTitle}</p>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <Button
                    ...
                    disabled={saving}
                  >
                    {t('dashboard.blogPostTags.delete')}
                  </Button>
                </div>
              </li>
            ))}
