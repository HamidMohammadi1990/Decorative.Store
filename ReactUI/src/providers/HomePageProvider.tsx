...
export function HomePageProvider({ children }: { children: ReactNode }) {
  ...
  const contentSlug = resolveCmsContentSlug(location.pathname)

  // Blog uses BlogMainNav; all other shop routes keep the full category menu.
  ...
  const cmsSlug = isAboutRoute(location.pathname)

    ? 'about'

    : isContactRoute(location.pathname)

      ? 'contact'

      : contentSlug ?? (isBlogRoute(location.pathname) ? 'blog' : 'shop')
  ...
  const load = useCallback(
    ...
    [locale, skipCatalogNav, skipHomeCatalogContent, skipCmsPage, cmsSlug],
